...
- **Everything granted goes to the Moddy Team role, and only to it** — never to
  a staff member's account, never to a role the server uses for anything else.
...
the requested permission bitfield and the requester's id, so a card answered
three weeks and two restarts later behaves exactly like a fresh one. See
...
_CID_PICK = r"moddy:teamaccess:pick:(?P<requester>\d{1,20}):(?P<perms>\d{1,20})"
_CID_SEND = r"moddy:teamaccess:send:(?P<requester>\d{1,20}):(?P<perms>\d{1,20})"
_CID_DECIDE = (r"moddy:teamaccess:(?P<action>accept|refuse):"
               r"(?P<requester>\d{1,20}):(?P<perms>\d{1,20})")
...
class TeamAccessSelect(ui.DynamicItem[ui.Select], template=_CID_PICK):
    ...
    def __init__(self, requester_id: int, value: int = 0, locale: str = "en-US"):
        ...
        super().__init__(
            ui.Select(
                custom_id=f"moddy:teamaccess:pick:{requester_id}:{value}",
...
class TeamAccessSendButton(ui.DynamicItem[ui.Button], template=_CID_SEND):
    ...
    def __init__(self, requester_id: int, value: int = 0, locale: str = "en-US"):
        super().__init__(
            ui.Button(
                ...
                custom_id=f"moddy:teamaccess:send:{requester_id}:{value}",
                disabled=not value,
            )
        )
        self.requester_id = requester_id
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        role = await find_team_role(bot, guild)
        if not role:
            await interaction.response.edit_message(view=design.error(
                t("staff.team.access.no_role_title", locale=locale),
                t("staff.team.access.no_role", locale=locale, name=f"**{TEAM_ROLE_NAME}**"),
            ))
            return
        ...
        card = await build_access_card(
            ...
            locale=card_locale, state=STATE_PENDING,
        )
        ...
        logger.info("Staff %s requested %s in guild %s", self.requester_id,
                    ",".join(keys), guild.id)
...
class TeamAccessPickerView(BaseView):
    ...
    def __init__(self, requester_id: int = 0, value: int = 0, locale: str = "en-US"):
        ...
        container.add_item(ui.TextDisplay(
            ...
            f"{t('staff.team.access.picker_desc', locale=locale, name=f'**{TEAM_ROLE_NAME}**')}"
        ))
        if keys:
...
class TeamAccessDecisionButton(ui.DynamicItem[ui.Button], template=_CID_DECIDE):
    ...
    def __init__(self, action: str, requester_id: int, value: int = 0,
        ...
        super().__init__(
            ui.Button(
                ...
                custom_id=f"moddy:teamaccess:{action}:{requester_id}:{value}",
            )
        )
        self.action = action
        self.requester_id = requester_id
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            await role.edit(
                permissions=wanted,
                reason=(f"Moddy Team access granted by {actor} ({actor.id}) — "
...
async def build_access_card(bot, guild: discord.Guild, *, requester_id: int,
    ...
    container.add_item(ui.TextDisplay(
        ...
        f"{t('staff.team.access.body', locale=locale, user=who, name=f'**{TEAM_ROLE_NAME}**')}"
    ))
    container.add_item(ui.Separator(spacing=discord.SeparatorSpacing.small))
    container.add_item(ui.TextDisplay(
    ...
    if state == STATE_PENDING:
        container.add_item(ui.TextDisplay(
            f"-# {t('staff.team.access.notice', locale=locale, name=f'**{TEAM_ROLE_NAME}**')}"))
    elif state == STATE_GRANTED:
        target = role.mention if role else f"**{TEAM_ROLE_NAME}**"
        container.add_item(ui.TextDisplay(
