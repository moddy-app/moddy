"""`/mod altguard unverify` — send a member back behind the AltGuard gate."""
...
@staff_command
class AltGuardUnverifyCommand(StaffCommand):
    command_type = CommandType.MODERATOR
    group = GROUP
    group_description = GROUP_DESCRIPTION
    name = "unverify"
    permission = PERMISSION
    description = "Revoke a member's AltGuard verification on a server."
    options = [
        SlashOption("user", "string", "User id or mention.", required=True),
        SlashOption("guild", "string", "Server id where the member is verified.", required=True),
    ]
    ...
    async def execute(self, ctx):
        ...
        await module.unverify_member(member, actor_id=ctx.author.id, moddy_staff=True)

        name = await format_member_name(ctx.bot, member)
        await ctx.send(view=design.success(
            t("staff.mod.altguard.unverified_title", locale=ctx.locale),
            t("staff.mod.altguard.unverified", locale=ctx.locale,
