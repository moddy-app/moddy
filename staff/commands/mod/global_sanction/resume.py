"""`/mod global resume` — restart a halted countdown (the appeal was refused).

The counterpart of `/mod global halt`. Filing an appeal freezes the deferred
consequences; refusing it has to unfreeze them, otherwise a rejected appeal
would protect the subject forever. The subject gets a **fresh** grace period —
they are told the new deadline, and it is their last warning before the
measures apply.

Accepting an appeal is `/mod global lift`, not this.
"""

import discord

from staff.framework import StaffCommand, SlashOption, staff_command, design, CommandType
from staff.commands.mod.global_sanction._shared import (
    GROUP, GROUP_DESCRIPTION, resolve_group, group_level, not_found,
)
from utils.global_sanction_views import GlobalSanctionResumeModal, build_group_panel
from utils.i18n import t


@staff_command
class GlobalResumeCommand(StaffCommand):
    command_type = CommandType.MODERATOR
    group = GROUP
    group_description = GROUP_DESCRIPTION
    name = "resume"
    permission = "global_enforcement"
    description = "Restart the enforcement countdown of a group (appeal refused)."
    options = [
        SlashOption("target", "string",
                    "Group id, or any case reference inside it.", required=True),
    ]

    async def execute(self, ctx):
        group_id, cases = await resolve_group(ctx.bot, ctx.opt("target"))
        if not group_id:
            await ctx.send(view=not_found(ctx.locale))
            return

        enforcement = await ctx.bot.db.get_enforcement(group_id)
        if not enforcement or enforcement.get("status") != "halted":
            await ctx.send(view=design.warning(
                t("global_sanctions.staff.resume_failed_title", locale=ctx.locale),
                t("global_sanctions.staff.resume_failed", locale=ctx.locale),
            ))
            return

        locale = ctx.locale
        bot = ctx.bot
        summary = [{
            "id": str(c["id"]), "reference": c["reference"],
            "subject_type": str(c["subject_type"]), "subject_id": str(c["subject_id"]),
        } for c in cases]
        level = group_level(cases)

        async def _on_done(interaction: discord.Interaction, row):
            await interaction.followup.send(view=build_group_panel(
                bot=bot, level=level, group_id=group_id, cases=summary,
                enforcement=row, notified=bool(row.get("notified")),
                locale=locale,
            ), ephemeral=True)

        await ctx.open_modal(
            lambda: GlobalSanctionResumeModal(
                bot=bot, group_id=group_id, locale=locale, on_done=_on_done,
            ),
            label=t("global_sanctions.staff.resume_button", locale=locale),
            prompt_title=t("global_sanctions.staff.resume_title", locale=locale),
            prompt_description=t("global_sanctions.staff.resume_prompt", locale=locale),
        )
