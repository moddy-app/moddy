...
|---|---|---|
| `moddy:bot` | Backend → Bot | Generic events: `config_updated`, `module_updated`, `settings_updated`, `premium_activated`, `payment_failed`, `stripe_action_result`, … (`bot.py::_handle_bot_event`). `settings_updated` covers `guilds.data.settings` (today: the server language, see [SERVER_LANGUAGE.md](SERVER_LANGUAGE.md)) — it drops the cached value and re-posts the panels written in that language. A config event carrying a `module_id` also re-applies that module's Discord side (panel, permissions) and acks on `moddy:dashboard` — see [ALTGUARD_INTEGRATION.md §5](ALTGUARD_INTEGRATION.md). `stripe_action_result` is the signed reply to a `stripe_action` request below, routed to `bot.stripe_admin.handle_reply()` and correlated by `request_id` |
| `moddy:subscription:updates` | Backend → Bot | Premium subscription cache invalidation + DM triggers (`bot.py::_handle_subscription_event`), incl. `notify_invoice` — one Stripe invoice, trials at 0 included (`services/invoice_notifier.py`, see [SUBSCRIPTION_SCHEMA.md](SUBSCRIPTION_SCHEMA.md) §3) |
| `moddy:blacklist:updates` | Backend/Bot → Bot | Global-sanction cache invalidation (`bot.py::_handle_blacklist_event`) |
| `moddy:dashboard` | Bot → Backend | Bot-originated notifications to the dashboard, incl. `social_subscribe_result`, `module_config_applied`, `create_stripe_customer` (`/manage stripecreate` — fire-and-forget, backend listener in `app/redis/stripe_events.py` creates the Stripe customer, idempotent on `stripe_customer_id`), `stripe_action` (`/manage stripe cancel/resume/refund/trial` — HMAC-SHA256 signed with `TASK_STREAM_SECRET`, replied to on `moddy:bot` as `stripe_action_result` correlated by `request_id`; see `services/stripe_admin_client.py` for the exact canonicalization/signing contract, which mirrors but is distinct from the `moddy:tasks` one in [TASK_SIGNATURE.md](TASK_SIGNATURE.md)) etc. |
...
| `moddy:staff` | Bot → Backend | A staff rank changed: `staff_ranked` / `staff_unranked` / `staff_updated` (`services/staff_events.py`). Feeds Discord **linked roles** — the backend re-reads `staff_permissions` and republishes the `team` boolean, so the message must go out **after** the write. It carries an id, never a state: `roles` is context for the backend logs, and a forged message can only trigger a republication that recomputes the truth. Fire-and-forget; a lost one is caught by the backend's 6 h resync — see [LINKED_ROLES.md](LINKED_ROLES.md) |
